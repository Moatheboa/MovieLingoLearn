...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path == "/getdata":
            ...
            scene = db_get.get_current_scene(user, 1) or 1  # The scene nr that the user is to work on. Second parameter 1 is movie_id. If first time, database won't have a current_scene and scene is set to 1.
        ...
        if self.path == "/":  # If the post is from the guess form
            ...
            if user_answer == right_answer:
                ...
                if scene > db_post.get_nr_of_lines(1):  # 1 is for movie_id 1, for now it is not dynamic since we only have one movie
                    db_post.update_scene_count(user, 1, 1)  # The scene for the user is reset to 1 so user can start over and practice again.
